...
import { Shield, Calendar, User, ArrowLeft, MapPin, Car, CheckCircle, Search, AlertTriangle } from "lucide-react"
...
export default function ProtectorApp() {
  ...
  const loadAgents = async (serviceType?: string) => {
    try {
      const { data, error } = await ServicesAPI.getAvailableAgents(serviceType)
      if (data && !error) {
        setAgents(data)
      }
    } catch (error) {
  ...
  const loadVehicles = async (vehicleType?: string, capacity?: number) => {
    try {
      const { data, error } = await ServicesAPI.getAvailableVehicles(vehicleType, capacity)
      if (data && !error) {
        setVehicles(data)
      }
    } catch (error) {
  ...
  // Calculate pricing
  const calculatePrice = async (service: Service, duration: number) => {
    try {
      const { data, error } = await ServicesAPI.getServicePricing(
        service.id,
        duration,
        bookingData.pickup_coordinates.x > 0 ? bookingData.pickup_coordinates : undefined
      )
      return data
    } catch (error) {
