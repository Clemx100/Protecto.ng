...
export default function ProtectorApp() {
  ...
  const [authStep, setAuthStep] = useState("login") // "login", "register", "profile", "email-confirmation"
  ...
  const handleNextStep = () => {
    if (selectedService === "armed-protection") {
      if (bookingStep === 1) setBookingStep(2)
        ...
        // Initialize chat with first message containing full request summary
        const initialMessage = {
          id: 1,
          sender: "system",
          message: `New Protection Request - ${payload.id}`,
          timestamp: new Date().toISOString(),
      ...
      if (bookingStep === 1) setBookingStep(4)
        ...
        const initialMessage = {
          id: 1,
          sender: "system",
          message: `New Transportation Request - ${payload.id}`,
          timestamp: new Date().toISOString(),
          isRequestSummary: true,
          payload: payload,
        }
    ...
    try {
      if (authStep === "login") {
        ...
        if (data.user && !data.user.email_confirmed_at) {
          setAuthStep("email-confirmation")
          setAuthSuccess("Account created! Please check your email and click the confirmation link to continue.")
        } else {
          setAuthStep("profile")
          setAuthSuccess("Account created! Please complete your profile.")
        }
        ...
        if (user) {
          ...
          console.log('Profile data:', {
            ...
            full_name: `${authForm.firstName.trim()} ${authForm.lastName.trim()}`,
          ...
          console.log('Profile save result:', { profileData, error })
        ...
        setAuthSuccess("Registration completed successfully! Welcome to Protector.ng")
            ...
            <div className="space-y-6 text-center">
              ...
              <div>
                ...
                <p className="text-gray-400 mb-4">
                  We've sent a confirmation link to <strong>{authForm.email}</strong>
                </p>
                ...
                <button
                  onClick={async () => {
                    ...
                    if (error) {
                      ...
                      setAuthSuccess("Confirmation email sent again!")
                    }
                  }}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => handleAuthInputChange("emergencyPhone", e.target.value)}
                  className="w-full p-4 bg-gray-800/90 text-white rounded-lg border border-gray-700 focus:border-blue-500 focus:outline-none backdrop-blur-sm"
                />
              </div>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-900 rounded-lg p-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-white font-bold text-2xl">{returnTotalPrice()}</span>
                  </div>
                </div>
        ...
        {showChatThread && (
          <div className="flex flex-col h-full">
            ...
            <div className="p-4 border-b border-gray-800 bg-gray-900">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-semibold text-white">Request Chat</h2>
                        ...
                        <div className="text-sm space-y-2 bg-gray-900 p-3 rounded">
                          <div>
                            ...
                            {msg.payload.serviceType === "car-only"
                              ? "Car Transportation Only"
                              : `${msg.payload.protectionType} Protection Service`}
                          </div>
                          <div>
                            <strong>Pickup:</strong> {msg.payload.pickupDetails.location}
                          </div>
                          <div>
                            <strong>Date & Time:</strong> {msg.payload.pickupDetails.date} at{" "}
                            {msg.payload.pickupDetails.time}
                          </div>
                          <div>
                            <strong>Duration:</strong> {msg.payload.pickupDetails.duration}
                          </div>
                          <div>
                            <strong>Destination:</strong> {msg.payload.destinationDetails.primary}
                          </div>
                          ...
                          <div>
                            <strong>Contact:</strong> {msg.payload.contact.phone}
                          </div>
                          <div>
                            <strong>Total Price:</strong> ₦{msg.payload.totalPrice?.toLocaleString()}
                          </div>
                        </div>
                        <div className="text-xs text-gray-400">
                      ...
                      <div>
                        ...
                        {msg.hasInvoice && (
                          <div className="mt-2 p-2 bg-gray-800 rounded border">
                            ...
                            <div className="text-sm">Amount: ₦{bookingPayload?.totalPrice?.toLocaleString()}</div>
                            <Button
